...
void ble_hid_demo_task_kbd(void *pvParameters)
{
    ...
    ESP_LOGI(
        TAG,
        "GPIO4 configurato come INPUT con pull-up"
    );
    ...
    void *event_data)
{
    ...
    case ESP_HIDD_START_EVENT:
        ...
        if (param->start.status == ESP_OK) {
            ...
            ESP_LOGI(
                TAG,
                "Setting to connectable, discoverable"
            );
    ...
    case ESP_HIDD_CONNECT_EVENT:
        ...
        if (param->connect.status == ESP_OK) {
            ...
            ESP_LOGI(
                TAG,
                "Setting to non-connectable, non-discoverable"
            );
    ...
    case ESP_HIDD_DISCONNECT_EVENT:
        ...
        if (param->disconnect.status == ESP_OK) {
            ...
            ESP_LOGI(
                TAG,
                "Setting to connectable, discoverable again"
            );
...
static void esp_sdp_cb(
    esp_sdp_cb_event_t event,
    ...
    case ESP_SDP_INIT_EVT:
        ...
        if (param->init.status == ESP_SDP_SUCCESS) {
            ...
            esp_bluetooth_sdp_dip_record_t dip_record = {
                ...
                .vendor =
                    bt_hid_config.vendor_id,

                .vendor_id_source =
                    ESP_SDP_VENDOR_ID_SRC_BT,

                .product =
                    bt_hid_config.product_id,

                .version =
                    bt_hid_config.version,
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(
        esp_sdp_register_callback(esp_sdp_cb)
    );
